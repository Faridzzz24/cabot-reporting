...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $trendYear = (int) $request->input('trend_year', now()->year);
        $trendStart = (int) $request->input('trend_start', 1);
        $trendEnd = (int) $request->input('trend_end', 6);
        ...
        $monthlyTrend = [];
        for ($month = $trendStart; $month <= $trendEnd; $month++) {
            $date = \Carbon\Carbon::createFromDate($trendYear, $month, 1);
            $monthlyTrend[] = [
                'month' => $date->translatedFormat('M Y'),
                'count' => IncidentReport::whereYear('created_at', $trendYear)
