...
<div class="max-w-4xl mx-auto">
    <div class="glass-card p-6 mb-8 animate-fade-in-up">
        ...
        <form method="POST" action="{{ route('users.store') }}" class="grid sm:grid-cols-2 gap-4">
            @csrf
            <div>
                <label class="text-xs text-gray-500 mb-1 block">{{ __('Nama') }}</label>
                <input type="text" name="name" value="{{ old('name') }}" placeholder="{{ __('Nama lengkap') }}" class="form-input-dash w-full px-3 py-2.5 text-sm" required>
            ...
            <div>
                <label class="text-xs text-gray-500 mb-1 block">{{ __('Password') }}</label>
                <input type="password" name="password" placeholder="{{ __('Min. 8 karakter') }}" class="form-input-dash w-full px-3 py-2.5 text-sm" required>
                @error('password') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="text-xs text-gray-500 mb-1 block">{{ __('Role') }}</label>
                <select name="role" class="form-input-dash w-full px-3 py-2.5 text-sm" required>
                    <option value="supervisor">{{ __('Supervisor') }}</option>
                    <option value="she_officer">{{ __('SHE Officer') }}</option>
    ...
    <div class="glass-card overflow-hidden animate-fade-in-up" style="animation-delay: 0.1s">
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="text-sm font-semibold text-gray-900">{{ __('Daftar User') }} ({{ $users->count() }})</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Nama') }}</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Email') }}</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Role') }}</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell">{{ __('Dibuat') }}</th>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-5 py-4">
                            <span class="px-2 py-1 rounded-full text-xs font-semibold
                                ...
                                {{ $user->role === 'she_officer' ? 'bg-blue-100 text-blue-700' : '' }}
                        ...
                        <td class="px-5 py-4">
                            @if($user->id !== auth()->id())
                            <form method="POST" action="{{ route('users.destroy', $user) }}" onsubmit="return confirm('{{ __('Hapus user') }} {{ $user->name }}?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-xs text-gray-400 hover:text-red-500 transition-colors">{{ __('Hapus') }}</button>
                            </form>
                            @else
                            <span class="text-xs text-gray-300">{{ __('Anda') }}</span>
                            @endif
                        </td>
                    </tr>
                    @endforeach
